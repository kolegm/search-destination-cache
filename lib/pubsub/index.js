...
*/
ClientPubSub.prototype.on = function (channel, handler) {
  this._getReceiver().on('pmessage', function (pattern, channel, message) {
    if (pattern === channel) {
      handler(JSON.parse(message));
    }
  }.bind(this));

  this._getReceiver().psubscribe(this._prepareChannelScope(channel));
};
...
*/
ClientPubSub.prototype.off = function (channel) {
  this._getReceiver().punsubscribe(
...
ClientPubSub.prototype._initClient = function (options) {
  ...
  this.receiver.on('psubscribe', function (pattern, count) {
    if (options.debug_mode) {
      console.log(util.format(
        'Receiver subscribed to pattern \'%s\' with \'%s\' total subscriptions.',
        pattern,
        count
      ));
    }
  ...
  this.receiver.on('punsubscribe', function (pattern, count) {
    if (options.debug_mode) {
      console.log(util.format(
        'Receiver unsubscribed from pattern \'%s\' with \'%s\' total subscriptions.',
        pattern,
        count
      ));
    }
