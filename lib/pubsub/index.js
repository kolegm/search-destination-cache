...
ClientPubSub.prototype.cleanEvents = function () {
...
ClientPubSub.prototype._initClient = function (options) {
  // emitter
  {
    this.emitter = this._getClient(options);
  }
  
  // receiver for data
  {
    this.receiverData = this._getClient(options);
    this.receiverData.setMaxListeners(0);
    
    /*
    this.receiverData.on('subscribe', function (pattern, count) {
    ...
    */
    ...
    /*
    this.receiverData.on('unsubscribe', function (pattern, count) {
      //if (options.debug_mode) {
        console.log(util.format(
  ...
  // receiver for finish
  {
    this.receiverFinish = this._getClient(options);
    this.receiverFinish.setMaxListeners(0);
  
    /*
    this.receiverFinish.on('subscribe', function (pattern, count) {
      if (options.debug_mode) {
        console.log(util.format(
    ...
    */
    ...
    /*
    this.receiverFinish.on('unsubscribe', function (pattern, count) {
      if (options.debug_mode) {
        console.log(util.format(
...
ClientPubSub.prototype._checkReceiverData = function () {
  return _.isObject(this.receiverData);
}

ClientPubSub.prototype._checkReceiverFinish = function () {
  return _.isObject(this.receiverFinish);
}
...
ClientPubSub.prototype._checkReceiverDataWithException = function () {
  ...
  {
    throw new RedisError('Receiver data is not a valid');
  }
...
ClientPubSub.prototype._checkReceiverFinishWithException = function () {
  ...
  {
    throw new RedisError('Receiver finish is not a valid');
  }
