...
ClientPubSub.prototype.on = function (channel, handler) {
  ...
  this._getReceiver().subscribe(this._prepareChannelScope(channel));
};
...
ClientPubSub.prototype._initClient = function (options) {
  ...
  this.receiver.on('subscribe', function (pattern, count) {
  ...
  this.receiver.on('unsubscribe', function (pattern, count) {
    if (options.debug_mode) {
      console.log(util.format(
