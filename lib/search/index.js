...
Search.prototype._search = function (callback) {
  switch (this._getSearchType()) {
    case SEARCH_TYPE_ADDRESS:
      this._subscribeOnEvents(callback);
      sda.search({
    ...
    default:
      callback(new SearchError(util.format(
        'Could not start search process. Search type is \'%s\'',
        this._getSearchType()
      )));
  }
};
...
Search.prototype._subscribeOnEvents = function (callback) {
  this._subscribeOnData();
  this._subscribeOnFinish(callback);
};

Search.prototype._unsubscribeOnEvents = function () {
  setTimeout(function () {
...
Search.prototype._publishFinish = function () {
  pubsub.emit(
    ...
    util.format(
      'Search process has been finished. Searck key is \'%s\'',
      this._getSearchKey()
    )
  );
};
